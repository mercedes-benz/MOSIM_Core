...
#ifndef _THRIFT_CLIENT_BINARY_CPP_
#define _THRIFT_CLIENT_BINARY_CPP_

#include "ThriftClientBinary.h"
...
template<class T>
ThriftClientBinary<T>::ThriftClientBinary(std::string const & address, int port, bool autoOpen) :port(port), address(address)
{
...
template<class T>
ThriftClientBinary<T>::~ThriftClientBinary()
{
	this->transport->close();
}

template<class T>
void ThriftClientBinary<T>::Start()
{
	try {
